...
const Title = ({ children, className, ...rest }) => {
    ...
    return (
        <h1 style={{ perspective: 800, transformStyle: 'preserve-3d', filter: 'drop-shadow(30px 10px 4px black'}} ref={titleRef} className={`${className} text-5xl font-montserrat`} {...rest}>
            {children}
        </h1>
    );
};
